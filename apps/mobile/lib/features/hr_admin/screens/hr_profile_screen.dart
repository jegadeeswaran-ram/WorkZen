...
class HrProfileScreen extends ConsumerWidget {
  ...
  String _initials(String? name) {
    if (name == null || name.trim().isEmpty) return 'HR';
            ...
            child: Container(
              decoration: const BoxDecoration(
                gradient: LinearGradient(
                  ...
                  colors: [Color(0xFF0D2414), Color(0xFF0F1929), Color(0xFF080E1A)],
                  stops: [0.0, 0.55, 1.0],
                ),
                border: Border(bottom: BorderSide(color: Color(0x1F10B981))),
              ),
              child: SafeArea(
                bottom: false,
                child: Column(
                  children: [
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [BoxShadow(color: AppTheme.success.withValues(alpha: 0.5), blurRadius: 20, offset: const Offset(0, 6))],
                          ),
                    ...
                    Container(
                      ...
                      child: const Text('HR MANAGER', style: TextStyle(fontSize: 11, color: AppTheme.success, fontWeight: FontWeight.w700, letterSpacing: 0.8)),
                    ),
                    if (email.isNotEmpty) ...[
          ...
          SliverPadding(
            ...
            sliver: SliverToBoxAdapter(
              child: Column(
                ...
                children: [
                  const _HrProfileSectionTitle(title: 'Account Information'),
                  const SizedBox(height: 12),
                  _HrProfileInfoCard(
                    items: [
                      _HrProfileItem(icon: Icons.person_outline_rounded, label: 'Full Name', value: name),
                      if (email.isNotEmpty)
                        _HrProfileItem(icon: Icons.email_outlined, label: 'Email', value: email),
                      _HrProfileItem(icon: Icons.badge_outlined, label: 'Role', value: 'HR Manager'),
                      _HrProfileItem(icon: Icons.people_outline_rounded, label: 'Manages', value: 'Employees, Payroll, Compliance'),
                    ],
                  ),

                  const SizedBox(height: 24),
                  const _HrProfileSectionTitle(title: 'Settings'),
                  const SizedBox(height: 12),
                  _HrProfileActionCard(
                    items: [
                      _HrProfileAction(icon: Icons.lock_outline_rounded, label: 'Change Password', color: AppTheme.success, onTap: () {}),
                      _HrProfileAction(icon: Icons.notifications_none_rounded, label: 'Notification Preferences', color: AppTheme.warning, onTap: () {}),
                      _HrProfileAction(icon: Icons.help_outline_rounded, label: 'Help & Support', color: const Color(0xFF60A5FA), onTap: () {}),
                    ],
                  ),

                  const SizedBox(height: 24),
                  _HrLogoutButton(onTap: () => _showLogoutDialog(context, ref)),
...
class _HrProfileSectionTitle extends StatelessWidget {
  const _HrProfileSectionTitle({required this.title});
  final String title;
...
class _HrProfileInfoCard extends StatelessWidget {
  const _HrProfileInfoCard({required this.items});
  final List<_HrProfileItem> items;
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Icon(item.icon, size: 17, color: AppTheme.success),
                    ),
...
class _HrProfileActionCard extends StatelessWidget {
  const _HrProfileActionCard({required this.items});
  final List<_HrProfileAction> items;
...
class _HrLogoutButton extends StatelessWidget {
  const _HrLogoutButton({required this.onTap});
...
class _HrProfileItem {
  const _HrProfileItem({required this.icon, required this.label, required this.value});
  final IconData icon;
  final String label, value;
}

class _HrProfileAction {
  const _HrProfileAction({required this.icon, required this.label, required this.color, required this.onTap});
  final IconData icon;
  final String label;
  final Color color;
  final VoidCallback onTap;

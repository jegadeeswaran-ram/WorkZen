...
export default function EditQuotationPage() {
  ...
  const { control, register, handleSubmit, watch, reset, formState: { errors } } = useForm<FormValues>({
    ...
    defaultValues: {
      clientId: '', tenderId: '', issueDate: '', validUntil: '',
  ...
  useEffect(() => {
    ...
    reset({
      ...
      issueDate:       quotation.issueDate ? String(quotation.issueDate).split('T')[0] : '',
      validUntil:      quotation.validUntil ? String(quotation.validUntil).split('T')[0] : '',
      discount:        Number(quotation.discount ?? 0),
      notes:           quotation.notes ?? '',
      termsConditions: quotation.termsConditions ?? '',
      lineItems: (quotation.lineItems ?? []).length > 0
        ? (quotation.lineItems ?? []).map((li: any) => ({
  ...
  const saveMut = useMutation({
    ...
    onError: (e: any) => toast.error(e?.response?.data?.error?.message ?? 'Failed to save quotation'),
  });
  ...
  const taxable    = subtotal - Number(watchedDiscount);
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--wz-text-muted)' }}>
            {quotation.quotationNo} · {quotation.client?.name}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(d => saveMut.mutate(d))} className="space-y-5">
        ...
        <div className="glass-card p-5 grid grid-cols-2 gap-6 items-start">
          ...
          <div className="rounded-xl p-4 space-y-2"
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider mb-3" style={{ color: '#38bdf8' }}>Summary</p>
            {[
              { label: 'Subtotal',       val: subtotal,               color: 'var(--wz-text-primary)'   },
              { label: 'Discount',       val: -Number(watchedDiscount), color: '#f43f5e'                 },
              { label: 'Taxable Amount', val: taxable,                color: 'var(--wz-text-secondary)'  },
        ...
        <div className="glass-card p-5 grid grid-cols-2 gap-4">
          <F label="Notes">
            <textarea {...register('notes')} rows={4} className="input-field w-full resize-none"
              placeholder="Special conditions, delivery terms..." />
          </F>
        ...
        <div className="glass-card p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="submit" disabled={saveMut.isPending}
              className="btn-primary flex items-center gap-2 px-8" style={{ background: '#0ea5e9' }}>
              <Save size={14} />
              {saveMut.isPending ? 'Saving…' : 'Save Quotation'}
            </button>
          </div>
        </div>
